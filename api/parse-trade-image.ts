...
export default async function handler(
  ...
  try {
    ...
    const rateLimitResult = await checkRateLimit(userId, 'parse-trade-image');
    ...
    const { dataUrl, systemPrompt, userText, model = 'gpt-4o-mini' } = req.body;
    ...
    return res.status(500).json({ 
      error: 'Failed to parse trade image',
