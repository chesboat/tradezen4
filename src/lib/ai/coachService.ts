...
export class CoachService {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a trading coach. Give concise, actionable guidance grounded in the user\'s trades, notes, mood, and account rules. Prefer concrete numbers and guardrail suggestions. Avoid generic platitudes.',
          },
  ...
  private static localAnswer(question: string, context: CoachContext): string {
    ...
    if (/max\s*(daily)?\s*loss|loss limit|risk cap/i.test(question)) {
      ...
      return `Conservative cap: ${Math.round(suggested)} (≈2R). Moderate: ${Math.round(Math.min(firmCap || threeR, threeR))} (≈3R). Pick the stricter of firm cap${firmCap ? ` (${firmCap})` : ''} and your R‑based cap.`;
    }
    ...
    return `P&L today: ${pnlToday.toFixed(2)}. ${guardrail}. ${tradeNote}. Tip: define 1–2 micro‑goals and stop if you hit your loss cap or quality drops.`;
  }
}
